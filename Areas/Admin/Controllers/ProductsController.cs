...
namespace TamThaiTuSport.Areas.Admin.Controllers
{
    ...
    public class ProductsController : Controller
    {
        ...
        // GET /admin/products/getimages - API trả về danh sách ảnh có sẵn
        [HttpGet]
        ...
        public async Task<IActionResult> Create(Product product, IFormFile? imageFile, string? existingImageName)
        {
            ViewBag.Categories = await _db.Categories.Where(c => c.IsActive).OrderBy(c => c.SortOrder).ToListAsync();
            ...
            if (imageFile != null && imageFile.Length > 0)
            {
                ...
                {
                    TempData["Error"] = "❌ Ảnh sản phẩm vượt quá dung lượng tối đa cho phép (5MB).";
                    return View(product);
                }
                ...
                {
                    TempData["Error"] = "❌ Định dạng ảnh không hợp lệ. Chỉ chấp nhận các định dạng: .jpg, .jpeg, .png, .gif, .webp, .avif.";
                    return View(product);
                }
            ...
            else if (!string.IsNullOrEmpty(existingImageName))
            {
                // Sử dụng ảnh từ thư viện có sẵn
                var safeFileName = Path.GetFileName(existingImageName);
                var imagePath = Path.Combine(_env.WebRootPath, "uploads", "products", safeFileName);
            ...
            if (await _db.Products.AnyAsync(p => p.Slug == product.Slug))
            {
                TempData["Error"] = "❌ Tên sản phẩm hoặc Slug này đã tồn tại, vui lòng thay đổi.";
                return View(product);
            }
        ...
        [HttpPost, ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, Product product, IFormFile? imageFile, string? existingImageName)
        {
            ViewBag.Categories = await _db.Categories.Where(c => c.IsActive).OrderBy(c => c.SortOrder).ToListAsync();
            ...
            var slug = string.IsNullOrEmpty(product.Slug) ? GenerateSlug(product.Name) : product.Slug;
            ...
            if (imageFile != null && imageFile.Length > 0)
            {
                ...
                {
                    var oldImagePath = Path.Combine(uploadsDir, existing.Image);
            ...
            else if (!string.IsNullOrEmpty(existingImageName))
            {
                // Sử dụng ảnh từ thư viện có sẵn
                var safeFileName = Path.GetFileName(existingImageName);
                var imgPath = Path.Combine(_env.WebRootPath, "uploads", "products", safeFileName);
                if (System.IO.File.Exists(imgPath))
                    existing.Image = safeFileName;
            }
